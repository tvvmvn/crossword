export default class PuzzleGenerator {
  ...
  createPuzzle(words) {
    ...
    for (let r = 0; r < this.canvas.length; r++) {
      board[r] = []
      for (let c = 0; c < this.canvas[r].length; c++) {
        board[r][c] = {
          id: 'cell' + r + c,
          label: 0,
          space: [0, 0],
          value: this.canvas[r][c],
          q: '',
          active: this.canvas[r][c] ? true : false,
          around: {
            top: r > 0 && this.canvas[r - 1][c],
            bottom: r < this.canvas.length - 1 && this.canvas[r + 1][c],
            left: c > 0 && this.canvas[r][c - 1],
            right: c < this.canvas[r].length - 1 && this.canvas[r][c + 1],
          }
        }
      }
    }
