...
class Caption {
  constructor(label, down, content, word) {
    this.label = label;
    this.down = down;
...
export function createPuzzle() {
  ...
  try {
    for (let j = 0; j < d.length; j++) {
      ...
      let word = getWord(minLength, maxLength, targetIndex, targetValue)
      ...
      if (down) {
        for (let i = r; i < r + word.name.length; i++) {
          board[i][c].value = word.name[i - r];
          board[i][c].active = true;
        }
      } else {
        for (let i = c; i < c + word.name.length; i++) {
      ...
      // add caption
      captions.push(new Caption(label, down, word.meaning, word.name))
    }
